...
def generate_profiling_sql():
    ...
    for i in range(0, len(cols_to_profile), batch_size):
        ...
        for col in batch:
            ...
            val_expression = f"CAST({col_name} AS STRING)" if col_type != 'STRING' else col_name
        ...
        SELECT
            ...
            ARRAY_AGG(STRUCT({val_expression} as value, count) ORDER BY count DESC LIMIT 5) as top_values
        FROM (
            SELECT {val_expression} as val, COUNT(*) as count
