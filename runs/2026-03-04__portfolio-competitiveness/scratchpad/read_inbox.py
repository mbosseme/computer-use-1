...
msgs = client.get("me/messages", params={"$top": 10, "$orderby": "receivedDateTime desc", "Prefer": 'outlook.body-content-type="text"'}, headers={"Prefer": 'outlook.body-content-type="text"'})
for m in msgs.get("value", []):
    print(m.get('subject'))
    print(m.get('body', {}).get('content', '')[:1000])
    print("====================")
