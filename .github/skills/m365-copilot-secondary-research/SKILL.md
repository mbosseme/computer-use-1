...
4. Set model selector to **GPT-5.2 Think** (or closest available equivalent), and re-check after navigation refresh.
   - **How to select**: Click the model/mode dropdown button (e.g., "Think deeper" or current mode label) → click **"More"** menuitem in the dropdown → select **"GPT-5.2 Think deeper"** from the submenu. The button should then display **"GPT-5.2 Think"**.
